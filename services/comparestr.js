...
exports.createJob = async (req, next, queue, user, email) => {
    ...
    const sessionJob = await CompareSTRJob.startSession();
    const sessionModel = await CompareSTR.startSession();
    ...
    try {
        if (req.user) {
            ...
            newJob = await CompareSTRJob.create([req.body], { session: sessionJob });
        }
        ...
        await CompareSTR.create([req.body], { session: sessionModel });
