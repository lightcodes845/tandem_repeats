...
exports.createJob = async (req, next, queue, user, email) => {
  ...
  const sessionJob = await DumpSTRJob.startSession();
  const sessionModel = await DumpSTR.startSession();
  ...
  try {
    if (req.user) {
      ...
      newJob = await DumpSTRJob.create([req.body], { session: sessionJob });
    }
    ...
    await DumpSTR.create([req.body], { session: sessionModel });
