...
exports.createJob = async (req, next, queue, user, email) => {
    ...
    const sessionJob = await StatSTRJob.startSession();
    const sessionModel = await StatSTR.startSession();
    ...
    try {
        if (req.user) {
            ...
            newJob = await StatSTRJob.create([req.body], { session: sessionJob });
        }
        ...
        await StatSTR.create([req.body], { session: sessionModel });
