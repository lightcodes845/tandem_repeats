...
exports.createJob = async (req, next, queue, user, email) => {
    ...
    const sessionJob = await GangSTRJob.startSession();
    const sessionModel = await GangSTR.startSession();
    ...
    try {
        if (req.user) {
            ...
            newJob = await GangSTRJob.create([req.body], { session: sessionJob });
        }
        ...
        await GangSTR.create([req.body], { session: sessionModel });
