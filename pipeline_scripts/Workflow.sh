...
refbias_xrange_max_qcstrA=$9
 ...
 # data preparation
    ...
    ${bin_dir}/qcSTR --vcf $vcf_qcstrA --vcftype hipstr --quality per-locus --quality per-sample --quality locus-stratified --quality sample-stratified --quality per-call "${args[@]}" --out ${output_dir}/qc
...
#hipstr call-level filters
	...
	${bin_dir}/dumpSTR --vcf $vcf_dumpstr --vcftype hipstr --zip --use-length "${args[@]}" --out ${output_dir}/dump
...
vcf_qcstrB=${output_dir}/dump.vcf.gz # Use the output VCF from dumpstr
    ...
    ${bin_dir}/qcSTR --vcf $vcf_qcstrB --vcftype hipstr --quality per-locus --quality per-sample --quality locus-stratified --quality sample-stratified --quality per-call "${args[@]}" --out ${output_dir}/qc
else
...
${bin_dir}/statSTR --vcf $vcf --vcftype hipstr --thresh --afreq --acount --nalleles --hwep --het --entropy --mean --mode --var --numcalled "${args[@]}" --out ${output_dir}/stat
else
